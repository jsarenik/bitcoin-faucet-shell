...
net=$(hnet.sh)
tmp=$(mktemp /dev/shm/catapultleft-$net-XXXXXX) || exit 1
...
num=$(wc -l < $list)
cd $wd
#awk '($3>0.01){print $1, $2, $3}' $list | while read txid vout amount rest; do
...
# echo $txid $vout $amount | awklist.sh -f $fee -d tb1pfees9rn5nz -a 1234 \
#  | mktx.sh | crt.sh | srt.sh | safecat.sh $lh
#sert.sh <$lh
#done
